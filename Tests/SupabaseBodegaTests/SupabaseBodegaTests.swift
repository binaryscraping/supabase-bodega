...
final class SupabaseStorageEngineTests: XCTestCase {
  ...
  func testWritingDataAndKeysSucceeds() async throws {
    ...
    let readKeysAndObjects: [(key: CacheKey, data: Data)] = await storage.readAllDataAndKeys()
      .sorted(by: {
        String(data: $0.data, encoding: .utf8)! < String(data: $1.data, encoding: .utf8)!
      })
  ...
  func testReadingArrayOfDataSucceeds() async throws {
    try await writeItemsToDatabase(count: 10)
    let keyCount = await storage.keyCount()
    XCTAssertEqual(keyCount, 10)

    let firstTwoValues = await storage
      .read(keys: [CacheKey(verbatim: "0"), CacheKey(verbatim: "1")])
...
extension SupabaseStorageEngineTests {
  fileprivate static let testData = Data("Test".utf8)
  fileprivate static let testCacheKey = CacheKey(verbatim: "test-key")

  fileprivate static let storedKeysAndData: [(key: CacheKey, data: Data)] = [
  ...
  private func writeItemsToDatabase(count: Int) async throws {
    let dataAndKeys = (0 ..< count).map {
      (CacheKey(verbatim: "\($0)"), "Value \($0)".data(using: .utf8)!)
    }
